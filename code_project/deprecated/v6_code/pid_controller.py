...
class PIDController(Controller):
    def __init__(self, 
        ...
        self.aw_beta    = float(anti_windup.get('back_calculation_beta', 0.0))                   # solo back-calculation
    ...
    def compute_action(self, state_dict: Dict[str, Any]) -> float: # 'current_system_state'
        try:
            ...
            if self.clipping_output:
                ...
                control_action = self.limited_control_action
            if self.normalize_output:
                # La normalización se aplica sobre la acción ya clipeada
                self.normalize_control_action = (self.raw_control_action - self.actuator_min) / self.actuator_range
                control_action = self.normalize_control_action
            
            # El método devuelve la acción procesada localmente.
            return float(control_action)
        
        except Exception as e_compute: 
            logger.error(f"[PIDController:compute_action] Unexpected error: {e_compute}", exc_info=True)
            return 0.0 # 'e_compute'
    ...
    def _update_integral_term(self):
        ...
        if self.aw_enabled and self.aw_method == 'conditional':
            is_in_windup_state = \
                (self.last_effective_control_action >= self.actuator_max and self.current_error > 0) or \
                (self.last_effective_control_action <= self.actuator_min and self.current_error < 0)
        ...
        if self.aw_enabled and self.aw_method == 'back_calculation':
            correction = (self.aw_beta * self.last_saturation_error) / self.current_ki
