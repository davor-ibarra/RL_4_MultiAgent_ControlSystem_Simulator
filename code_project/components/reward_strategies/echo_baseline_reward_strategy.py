...
class EchoBaselineRewardStrategy(RewardStrategy): # Implementar Interfaz RewardStrategy
    ...
    """
    def __init__(self, **kwargs): # Aceptar kwargs por si se añaden params en el futuro
        logger.info("EchoBaselineRewardStrategy inicializada.")
        if kwargs:
             logger.warning(f"EchoBaselineRewardStrategy recibió parámetros inesperados: {kwargs}")
    ...
    def compute_reward_for_learning(
        ...
        interval_reward: float,                   # Ignorado
        ...
        **kwargs
        ...
        if reward_dict is None:
            logger.warning(f"EchoBaseline: reward_dict es None para '{gain}'. Usando 0.")
            return 0.0
        if not isinstance(reward_dict, dict):
             logger.warning(f"EchoBaseline: reward_dict no es dict para '{gain}'. Usando 0.")
             return 0.0

        diff_reward = reward_dict.get(gain)

        if diff_reward is None:
            logger.warning(f"EchoBaseline: Ganancia '{gain}' no en reward_dict: {reward_dict.keys()}. Usando 0.")
            reward_for_q_update = 0.0
        elif not isinstance(diff_reward, (float, int)) or pd.isna(diff_reward) or not np.isfinite(diff_reward):
             logger.warning(f"EchoBaseline: R_diff para '{gain}' inválido ({diff_reward}). Usando 0.")
             reward_for_q_update = 0.0
        else:
            reward_for_q_update = float(diff_reward)
            # logger.debug(f"EchoBaseline: Usando R_diff={reward_for_q_update:.4f} para '{gain}'.")
