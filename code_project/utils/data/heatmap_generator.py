...
class HeatmapGenerator:
    ...
    def __init__(self, logger_instance: logging.Logger):
        ...
        self._logger.info("HeatmapGenerator instance created.")
    ...
    def _load_and_prepare_data(self, results_folder: str) -> Optional[pd.DataFrame]:
        ...
        self._logger.info(f"Buscando archivos simulation_data_ep_*.json en: {results_folder}")
        try:
            ...
            if not files:
                self._logger.warning("No se encontraron archivos de datos detallados ('simulation_data_ep_*.json'). No se pueden generar datos de heatmap.")
                return None
            ...
            try:
                ...
                self._logger.warning("No se pudo ordenar los archivos de datos por número de episodio. Procesando en orden del sistema de archivos.")

            self._logger.info(f"Encontrados {len(files)} archivos de datos detallados para cargar.")

            # 1.4: Cargar datos de todos los archivos
            raw_episode_list_from_files: List[Dict] = []
            for filename in files:
                filepath = os.path.join(results_folder, filename)
                self._logger.debug(f"Cargando archivo: {filename}")
                try:
                    ...
                    if isinstance(episodes_in_file, list):
                        ...
                        if len(valid_episodes) != len(episodes_in_file):
                            self._logger.warning(f"Archivo {filename} contenía elementos que no eran diccionarios.")
                        raw_episode_list_from_files.extend(valid_episodes)
                        total_files_processed += 1
                    else:
                        self._logger.warning(f"Archivo {filename} no contiene una lista de episodios como se esperaba. Saltando archivo.")
                except json.JSONDecodeError as e_json:
                    self._logger.error(f"Error decodificando JSON en {filename}: {e_json}. Saltando archivo.")
                except Exception as e_load:
                    self._logger.error(f"Error cargando o procesando {filename}: {e_load}. Saltando archivo.")

            if not raw_episode_list_from_files:
                self._logger.warning("No se cargaron datos válidos de ningún archivo de episodios.")
                return None
            ...
            for i, episode_dict in enumerate(raw_episode_list_from_files):
                ...
                if not isinstance(time_values, list) or not time_values:
                    self._logger.warning(f"Episodio {episode_id_val} no tiene datos de 'time' válidos. Saltando episodio.")
                    continue
                ...
                for metric, values in episode_dict.items():
                    ...
                    if isinstance(values, list):
                        ...
                        if len(values) == num_steps:
                            ...
                            self._logger.debug(f"Ep {episode_id_val}: Métrica '{metric}' (len {len(values)}) desalineada con 'time' (len {num_steps}). Alineando...")
                ...
                try:
                    ...
                    self._logger.error(f"Error creando DataFrame para episodio {episode_id_val}: {e_df}")

            if not all_aligned_data:
                self._logger.warning("No se pudieron crear DataFrames alineados para ningún episodio.")
                return None
            ...
            self._logger.info(f"Datos cargados y aplanados/alineados: {total_timesteps_loaded} timesteps de {total_episodes_processed} episodios en {total_files_processed} archivos.")
            # Loguear columnas disponibles para depuración
            self._logger.debug(f"Columnas disponibles en DataFrame combinado: {df_combined.columns.tolist()}")
            return df_combined

        except OSError as e_os:
            self._logger.error(f"Error de sistema de archivos listando/accediendo a {results_folder}: {e_os}")
            return None
        except Exception as e_main:
            self._logger.error(f"Error inesperado durante carga/preparación de datos detallados: {e_main}", exc_info=True)
            return None
    ...
    def _calculate_heatmap_grid(self, df: pd.DataFrame, config: Dict) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if not all(k in config for k in required_cfg_keys):
                ...
                self._logger.error(f"Heatmap config (plot='{plot_name_for_log}') Faltan claves: {missing_keys}.")
                raise ValueError(f"Configuración de heatmap incompleta: faltan {missing_keys}")
            ...
            try:
                 ...
                 if num_bins <= 0:
                      self._logger.warning(f"Heatmap ('{plot_name_for_log}'): 'bins' debe ser > 0, recibido {num_bins_cfg}. Usando default 50.")
                      num_bins = 50
            except (ValueError, TypeError):
                 self._logger.warning(f"Heatmap ('{plot_name_for_log}'): 'bins' inválido ({num_bins_cfg}). Usando default 50.")
                 num_bins = 50
            ...
            logger.debug(f"Heatmap ('{plot_name_for_log}'): Límites config: x=[{xmin}, {xmax}], y=[{ymin}, {ymax}]") 
            ...
            if missing_cols:
                ...
                if 'termination_reason' in missing_cols and filter_reasons:
                    self._logger.warning(f"Heatmap ('{plot_name_for_log}'): Se especificó filter_termination_reason pero la columna 'termination_reason' falta en los datos detallados. El filtro no se aplicará.")
                ...
                if missing_essential:
                    self._logger.error(f"Heatmap ('{plot_name_for_log}'): Faltan columnas esenciales {missing_essential}. Columnas disponibles: {df.columns.tolist()}")
                    raise ValueError(f"Columnas esenciales para heatmap no encontradas: {missing_essential}")
            ...
            if filter_reasons and isinstance(filter_reasons, list) and 'termination_reason' in df_filtered.columns:
                ...
                self._logger.info(f"Heatmap ('{plot_name_for_log}'): Filtrado por {filter_reasons}. Datos: {rows_after_filter}/{rows_before_filter} timesteps.")
                if df_filtered.empty:
                    self._logger.warning(f"Heatmap ('{plot_name_for_log}'): No quedan datos después de filtrar por termination_reason. No se generará heatmap.")
                    return None
            elif filter_reasons and isinstance(filter_reasons, list) and 'termination_reason' not in df_filtered.columns:
                self._logger.warning(f"Heatmap ('{plot_name_for_log}'): filter_termination_reason especificado pero la columna no está presente. No se aplicará filtro.")
            ...
            for col in cols_to_convert:
                if col in df_filtered.columns:
                    ...
                    self._logger.error(f"Heatmap ('{plot_name_for_log}'): Columna '{col}' inesperadamente ausente antes de conversión numérica.")
            ...
            if df_numeric.empty:
                self._logger.warning(f"Heatmap ('{plot_name_for_log}'): Sin datos numéricos válidos después de la limpieza (NaN/inf).")
                return None
            self._logger.debug(f"Heatmap ('{plot_name_for_log}'): Datos numéricos válidos {rows_after_drop}/{rows_before_drop} para binning y agregación.")
            ...
            if df_filtered_for_binning.empty:
                logger.warning(f"Heatmap ('{plot_name_for_log}'): Sin datos válidos después de filtrar por límites ({', '.join(applied_filters)}).")
                return None
            if applied_filters:
                logger.debug(f"Heatmap ('{plot_name_for_log}'): Datos filtrados por límites ({', '.join(applied_filters)}) -> {rows_after_limit_filter}/{rows_before_limit_filter} para binning.")
            ...
            try:
                ...
                if (x_range_max - x_range_min) < tolerance_range:
                    logger.warning(f"Heatmap ('{plot_name_for_log}'): Rango X nulo o negativo ({x_range_min} a {x_range_max}) después de aplicar límites/filtrado. No se puede generar heatmap.")
                    return None
                if (y_range_max - y_range_min) < tolerance_range:
                    logger.warning(f"Heatmap ('{plot_name_for_log}'): Rango Y nulo o negativo ({y_range_min} a {y_range_max}) después de aplicar límites/filtrado. No se puede generar heatmap.")
                    return None
                ...
                logger.debug(f"Heatmap ('{plot_name_for_log}'): Bordes X definidos ({len(x_bin_edges)}): [{x_bin_edges[0]:.3f} ... {x_bin_edges[-1]:.3f}]")
                logger.debug(f"Heatmap ('{plot_name_for_log}'): Bordes Y definidos ({len(y_bin_edges)}): [{y_bin_edges[0]:.3f} ... {y_bin_edges[-1]:.3f}]")
                # Usar pd.cut para obtener los índices de los bins y los bordes explícitos
                self._logger.debug(f"Heatmap ('{plot_name_for_log}'): Aplicando pd.cut con bordes explícitos (num_bins={num_bins})")
                ...
                if df_numeric[[f'{x_var}_bin', f'{y_var}_bin']].isnull().any().any():
                    self._logger.warning(f"Heatmap ('{plot_name_for_log}'): Se encontraron NaNs en los índices de bin. Eliminando filas afectadas.")
                    df_numeric.dropna(subset=[f'{x_var}_bin', f'{y_var}_bin'], inplace=True)
                    if df_numeric.empty:
                        self._logger.warning(f"Heatmap ('{plot_name_for_log}'): Sin datos después de eliminar NaNs de bins.")
                        return None
                ...
                self._logger.error(f"Error durante el binning (pd.cut) para heatmap '{plot_name_for_log}': {e_cut}. Asegurar que 'bins' ({num_bins}) sea válido para el rango de datos.")
                return None
            except Exception as e_bin:
                self._logger.error(f"Error inesperado durante el binning para heatmap '{plot_name_for_log}': {e_bin}", exc_info=True)
                return None
            ...
            if not agg_func_pd:
                self._logger.warning(f"Heatmap ('{plot_name_for_log}'): Agregación '{agg_func_name}' no reconocida. Usando 'count'.")
                agg_func_pd = 'size'
            ...
            self._logger.debug(f"Creando pivot_table para '{plot_name_for_log}' (X:{x_var}, Y:{y_var}, Value:{value_var_for_pivot}, Agg:{agg_func_pd}) usando índices de bin...")
            try:
                ...
                self._logger.error(f"Error durante pd.pivot_table para heatmap '{plot_name_for_log}': {e_pivot}", exc_info=True)
                return None
            ...
            if x_bin_edges is None or y_bin_edges is None:
                self._logger.error(f"Heatmap ('{plot_name_for_log}'): Bin edges no están definidos después del binning. No se pueden calcular centros.")
                return None
            try:
                ...
                if len(x_centers) != num_bins or len(y_centers) != num_bins:
                    self._logger.warning(f"Heatmap ('{plot_name_for_log}'): Discrepancia entre num_bins ({num_bins}) y número de centros calculados (X:{len(x_centers)}, Y:{len(y_centers)}). Usando índices numéricos.")
                ...
                self._logger.warning(f"Heatmap ('{plot_name_for_log}'): Error asignando centros de bins como ejes: {e_repr}. Se usarán índices numéricos.", exc_info=True)
                # Nombrar ejes aunque se usen índices numéricos
                heatmap_table.index.name = y_var
                heatmap_table.columns.name = x_var

            self._logger.info(f"Cuadrícula heatmap ('{plot_name_for_log}') calculada ({heatmap_table.shape[0]}x{heatmap_table.shape[1]}) usando '{agg_func_pd}'.")
            return heatmap_table

        except ValueError as ve: # Capturar errores de validación específicos
            self._logger.error(f"Error de configuración o datos para heatmap '{plot_name_for_log}': {ve}")
            return None
        except Exception as e:
            self._logger.error(f"Error inesperado calculando cuadrícula heatmap '{plot_name_for_log}': {e}", exc_info=True)
            return None
    ...
    def generate(self, results_folder: str, heatmap_configs: List[Dict], output_excel_filepath: str):
        ...
        if df_all_data is None:
            self._logger.error("Fallo al cargar datos detallados. No se pueden generar datos para heatmaps.")
            return
        ...
        try:
            ...
            engine = 'openpyxl'
            self._logger.info("Usando engine 'openpyxl' para escribir Excel.")
        except ImportError:
            self._logger.warning("Biblioteca 'openpyxl' no encontrada. Se usará el engine por defecto de Pandas (puede ser limitado o requerir otras dependencias). Instalar: pip install openpyxl")
        ...
        for config in heatmap_configs:
            ...
            if plot_name:
                ...
                self._logger.warning(f"Heatmap config index={plot_index} no tiene 'name'. Usando nombre de hoja por defecto: '{sheet_name}'. Se recomienda añadir un 'name' único en la config YAML.")
            ...
            while sheet_name in processed_sheet_names:
                ...
                if count > 10: # Evitar bucle infinito si hay demasiados duplicados
                    self._logger.error(f"Demasiadas colisiones de nombre de hoja para '{original_sheet_name}'. Saltando este heatmap.")
                    sheet_name = None
                    break
            ...
            self._logger.info(f"Procesando config heatmap ({log_name_ref}) -> Hoja: '{sheet_name}'")
            ...
            try:
                ...
                self._logger.error(f"Fallo irrecuperable calculando grid para hoja '{sheet_name}'. Error: {e_calc}", exc_info=True)

            # 1.18: Añadir a la lista para escribir si es válido
            if grid_df is not None and not grid_df.empty:
                sheets_to_write[sheet_name] = grid_df
                generated_count += 1
                self._logger.info(f"Datos para hoja '{sheet_name}' generados correctamente ({grid_df.shape[0]}x{grid_df.shape[1]}).")
            else:
                self._logger.warning(f"No se generaron datos válidos para la hoja '{sheet_name}'. No se incluirá en el archivo Excel.")

        # 1.19: Escribir todas las hojas válidas al archivo Excel
        if generated_count > 0:
            self._logger.info(f"Escribiendo {generated_count} hojas de datos heatmap en -> {output_excel_filepath}")
            try:
                ...
                writer.close()
                self._logger.info("Archivo Excel con datos de heatmap guardado exitosamente.")
            except Exception as e_write:
                self._logger.error(f"Error escribiendo archivo Excel '{output_excel_filepath}': {e_write}", exc_info=True)
                ...
                if writer is not None and hasattr(writer, 'close'):
                    try:
                        ...
                        writer.close()
                        self._logger.info("ExcelWriter cerrado tras error de escritura.")
                    except Exception as e_close_err:
                        self._logger.error(f"Error cerrando ExcelWriter tras error de escritura: {e_close_err}")
            # finally: # No es necesario finally si usamos with, pero aquí no lo usamos
                # if writer is not None:
                #      try: writer.close()
                #      except: pass
        else:
            self._logger.warning("No se generaron hojas de datos heatmap válidas. No se creó el archivo Excel.")